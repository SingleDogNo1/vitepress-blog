...
function getCode(code: string, options?: VuePreviewPluginOptions) {
  ...
  if (options?.props?.previewBodyStyle)
    expandProps += ` previewBodyStyle="${encodeURIComponent(
      JSON.stringify(options.props.previewBodyStyle)
    )}"`
  if (options?.props?.previewAppStyle)
    expandProps += ` previewAppStyle="${encodeURIComponent(
      JSON.stringify(options.props.previewAppStyle)
    )}"`
  if (options?.props?.importMap)
    expandProps += ` importMap="${encodeURIComponent(JSON.stringify(options.props.importMap))}"`

  return code.trim()
    ? `<VuePreview code="${str}" ${expandProps} encode ssr></VuePreview>\n`
...
export default function vuePreviewPlugin(options?: VuePreviewPluginOptions) {
  return {
    ...
    transform(code: string, id: string) {
      if (!id.endsWith('.md') || !/```vue preview/i.test(code)) return code
      let startIndex
      let endIndex
